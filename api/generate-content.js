...
- heroTitle: 감성/시즌 + 혜택 구성, 국문 기준 공백 포함 최대 18자. 쉼표(,)가 포함될 경우 쉼표 뒤에 반드시 \\n을 넣어 줄바꿈 (예: "봄날의 설렘,\\nM몰이 선물해요" / "겨울 준비물,\\nM몰에서 완성하세요"). 쉼표 없이 자연스럽게 이어지는 문장도 가능. 퍼센트(%), 원(원/₩), 숫자 정량 표현 절대 금지.
- sectionName: 섹션을 표현하는 영문 대문자 1~2단어 (예: BENEFIT, SPECIAL, SEASON)
- textTitle: 혜택 또는 시즌 상황 설명, 국문 기준 공백 포함 최대 18자, 줄바꿈은 \\n. '+' '/' 'n' 같은 나열·수식 기호 사용 금지. 퍼센트(%), 원(원/₩), 숫자 정량 표현 절대 금지. 시즌 감성이나 혜택의 느낌을 추상적으로 표현 (예: 봄 인기 아이템 할인 / 지금만 누리는 특가)
...
JSON 형식으로만 응답 (코드블록, 설명 없이 순수 JSON만):
{
  ...
  "couponCond": "..."
    ...
    const styleGuide = `현재 문구의 톤·형식을 유지하되 다른 표현으로 바꿔줘. 같은 언어(한/영), 같은 문장 구조를 지켜야 해. 현재 문구와 완전히 동일한 텍스트는 절대 반환하지 마. 글자 수는 현재 ${charLen}자(${byteLen}바이트) 기준 ±20% 이내로 맞춰줘. 기획전 컨셉에 맞는 워딩을 사용하되 간결하게, 혜택 중심으로, 과장 표현과 느낌표는 사용하지 마. '+' '/' 'n' 같은 나열·수식 기호, 퍼센트(%), 숫자로 된 정량 표현(예: 30%, 5000원, 최대 N%)은 절대 사용하지 마. 쉼표(,) 뒤에는 반드시 \\n으로 줄바꿈해줘.`;
    const prompts = {
